...
class AVLNode {
    ...
    protected:
        ...
        static int getHeight(AVLNode *n) { return n ? n->height : -1; }
        void updateHeight() {
...
template <class Base>
void AVLNode<Base>::verifySearchOrder() const {
    if (left) {
 ...
 * an array of pointers to AVLNode objects as the insert/remove methods search
 * for the place to do their work.  This array of pointers represents the path
 ...
 * large the array of pointers needs to be, at its largest. An AVL tree with
 ...
 *
 * The rebalancePathToRoot() method takes an array of pointers to AVLNode
 * objects, and the number of pointers that are on the array. This array should
...
class AVLTree {
    ...
    protected:
        ...
        void rebalancePathToRoot(AVLNode<Base> **path, int numOnPath);
